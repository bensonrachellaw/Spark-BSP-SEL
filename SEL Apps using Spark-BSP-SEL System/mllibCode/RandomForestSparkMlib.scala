...
object RandomForestSparkMlib {
  def main(args: Array[String]): Unit = {
    ...
    val res: DataFrame = RFmodel.transform(TestingData)
    ...
    println(s"RandomForestAccuracy = ${accuracy}")
    println(s"RandomForestRecall = ${recall}")
    println(s"RandomForestPrecision = ${precision}")
    println(s"RandomForestF1 = ${f1}")
    spark.stop()
  }
}
