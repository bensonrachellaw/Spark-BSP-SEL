 ...
 */
case class BspFileSourceScanExec(
  ...
  private lazy val inputRDD: RDD[InternalRow] = {
    ...
    relation.bucketSpec match {
      ...
      case _ =>
        ...
        mycreateNonBucketdReadRDD(readFile, selectedPartitions, relation)
    }
  }
  ...
  private def createNonBucketedReadRDD(
    ...
    println("============bdi==========:fsRelation.fileFormat" + fsRelation.fileFormat.getClass)
    ...
    println("=============bdi=========== DataSourceScanExec-> createNonBucketedReadRDD openCostInBytes:" + openCostInBytes)
    println("=============bdi=========== DataSourceScanExec-> createNonBucketedReadRDD maxSplitBytes:" + maxSplitBytes)
    val partitions = new ArrayBuffer[FilePartition]
    val currentFiles = new ArrayBuffer[PartitionedFile]
    var currentSize = 0L
    println("=============bdi=========== DataSourceScanExec-> createNonBucketedReadRDD splitFiles:" + splitFiles.length)
    ...
    closePartition()
    println("=============bdi=========== DataSourceScanExec-> createNonBucketedReadRDD partitions:" + partitions.size)
    new FileScanRDD(fsRelation.sparkSession, readFile, partitions)
  }

  private def mycreateNonBucketdReadRDD(
                                         readFile: (PartitionedFile) => Iterator[InternalRow],
                                         selectedPartitions: Seq[PartitionDirectory],
                                         fsRelation: HadoopFsRelation): BspRDD[InternalRow] = {
    val splitFiles = selectedPartitions.flatMap { partition =>
      partition.files.flatMap { file =>
        val blockLocations = getBlockLocations(file)
        val hosts = getBlockHosts(blockLocations, 0, file.getLen)
    ...
    new BspRDD(new FileScanRDD(fsRelation.sparkSession, readFile, partitions))
  }
  ...
  override def doCanonicalize(): BspFileSourceScanExec = {
    BspFileSourceScanExec(
      relation,
